				...
				if (redisUrl && redisToken) {
					...
					if (raw) {
						...
						for (const [id, entry] of Object.entries(
							parsed as Record<string, { score: number; title: string; discogsId: number | null }>,
						)) {
						...
						if (changes.length > 0) {
							console.log(
								`[import-worker] ${changes.length} gem tier changes detected, ${notifCount} notifications created`,
							);
						}
					}
				}
