...
const SESSION_TYPE_OPTIONS: {
	...
	},
	{
		...
		inactiveClass:
			"text-on-surface-variant border-outline-variant/30 hover:border-outline-variant/60",
	},
];
...
export function CreateCrateForm({ onSuccess, onCancel }: CreateCrateFormProps) {
	...
	return (
		...
		>
			<div className="font-mono text-xs text-primary tracking-[0.15em] mb-1">[NEW_CRATE]</div>
			...
			<div>
				...
				/>
				{errors.date && (
					<p className="font-mono text-xs text-destructive mt-1">{errors.date.message}</p>
				)}
			</div>
