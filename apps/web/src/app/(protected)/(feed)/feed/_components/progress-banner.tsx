...
export function ProgressBanner({ discogsConnected, followingCount }: ProgressBannerProps) {
	...
	return (
		<div className="bg-surface-container-low rounded-lg p-6 mb-8 border border-outline-variant/10">
			...
			<div className="flex justify-between items-center mb-3">
				...
				<span className="font-mono text-xs text-primary">{completedSteps} / 3 COMPLETE</span>
			</div>

			{/* Progress bar */}
			<Progress
				value={(completedSteps / 3) * 100}
				className="[&_[data-slot=progress-track]]:h-1 [&_[data-slot=progress-track]]:bg-surface-container-high [&_[data-slot=progress-indicator]]:bg-primary"
			/>
			...
			<div className="flex flex-wrap gap-4 mt-4">
				...
				<div className="flex items-center gap-2">
					{step1Complete ? (
						<>
							...
							<span className="font-mono text-xs text-primary line-through">Connect Discogs</span>
						</>
					) : (
						<>
				...
				<div className="flex items-center gap-2">
					{step2Complete ? (
						<>
							<span className="material-symbols-outlined text-primary text-base">check_circle</span>
							<span className="font-mono text-xs text-primary line-through">Follow 3 diggers</span>
						</>
					) : (
						<>
							<span className="material-symbols-outlined text-on-surface-variant text-base">
							...
							<Link
								...
								className="font-mono text-xs text-on-surface hover:text-primary transition-colors"
							>
								{followingCount > 0 ? `Follow 3 diggers (${followingCount}/3)` : "Follow 3 diggers"}
							</Link>
						</>
					)}
				</div>
				...
				<div className="flex items-center gap-2">
					...
					<span className="font-mono text-xs text-outline line-through">Join a group</span>
