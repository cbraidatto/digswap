...
function InferredText({ value, confidence }: { value: string | null; confidence: "high" | "low" }) {
  ...
  return (
    <span className={confidence === "low" ? "italic text-on-surface-variant/60" : ""}>
      {value}
    </span>
  );
}
...
export default function BibliotecaPage() {
  ...
  if (state === "not-desktop") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        <span className="material-symbols-outlined text-[48px] text-on-surface-variant/40">desktop_windows</span>
  ...
  if (state === "scanning" && progress) {
    const percent = progress.filesFound > 0
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 px-8">
        <span className="text-on-surface-variant text-sm uppercase tracking-wider font-mono">Escaneando...</span>
        <div className="w-full max-w-lg">
          ...
          <div className="flex justify-between mt-2 text-xs text-on-surface-variant font-mono">
            <span>{progress.filesProcessed} / {progress.filesFound} arquivos</span>
            <span>{percent}%</span>
          </div>
        </div>
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 px-8">
        ...
        <div className="w-full max-w-lg max-h-48 overflow-y-auto bg-surface-container-high/50 rounded-lg p-4">
          {scanResult.skippedFiles.map((f) => (
            <div key={f.path} className="text-xs text-on-surface-variant py-1 border-b border-outline-variant/10 last:border-0">
  ...
  // Library view
  const albumGroups = viewMode === "album"
      ...
      {viewMode === "list" && (
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {tracks.map((track) => (
                <tr key={track.id} className="border-b border-outline-variant/5 hover:bg-surface-container-high/30 transition-colors">
                  <td className="py-2 px-3"><InferredText value={track.title} confidence={track.titleConfidence} /></td>
                  <td className="py-2 px-3"><InferredText value={track.artist} confidence={track.artistConfidence} /></td>
                  <td className="py-2 px-3"><InferredText value={track.album} confidence={track.albumConfidence} /></td>
                  <td className="py-2 px-3 font-mono text-xs uppercase text-on-surface-variant">{track.format}</td>
                  <td className="py-2 px-3 text-right font-mono text-xs text-on-surface-variant">{formatBitrate(track.bitrate)}</td>
                  <td className="py-2 px-3 text-right font-mono text-xs text-on-surface-variant">{formatDuration(track.duration)}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            return (
              <div key={albumName} className="bg-surface-container-high/20 rounded-xl p-4">
                <div className="flex items-center gap-3 mb-3 pb-3 border-b border-outline-variant/10">
                  <span className="material-symbols-outlined text-[28px] text-on-surface-variant/40">album</span>
                  <div>
                    ...
                    <p className="text-xs text-on-surface-variant">
                      <InferredText value={firstTrack.artist} confidence={firstTrack.artistConfidence} />
                ...
                <div className="space-y-0.5">
                  {albumTracks.map((track) => (
                    <div key={track.id} className="flex items-center py-1.5 px-2 rounded hover:bg-surface-container-high/30 text-sm">
                      <span className="w-8 text-right text-xs font-mono text-on-surface-variant/40 mr-3">
                        {track.trackNumber ?? "—"}
                      </span>
                      <span className="flex-1"><InferredText value={track.title} confidence={track.titleConfidence} /></span>
                      <span className="text-xs font-mono text-on-surface-variant ml-4">{formatDuration(track.duration)}</span>
                    </div>
                  ))}
                </div>
              </div>
