 ...
 * niche styles.
 */
export const DISCOGS_STYLES_BY_GENRE: Record<
  DiscogsGenre,
  readonly string[]
> = {
  Blues: [
...
export const DISCOGS_CONDITIONS = {
  ...
  "NM or M-": "Near Mint — Nearly perfect. Little or no signs of wear on record or cover. Plays perfectly with no issues.",
  ...
  "VG+": "Very Good Plus — Plays very clean with just a few occasional light pops or light static. Minor cosmetic marks only.",
...
export const DISCOGS_SLEEVE_CONDITIONS = {
  ...
  "VG+": "Very Good Plus — Shows some signs of handling. Possibly a very minor crease or light ring wear. No seam splits.",
...
export function getGenresForStyle(style: string): DiscogsGenre[] {
  return DISCOGS_GENRES.filter((genre) =>
    DISCOGS_STYLES_BY_GENRE[genre].includes(style),
  );
}
 ...
 */
export function isDiscogsConditionGrade(
  value: string,
): value is DiscogsConditionGrade {
