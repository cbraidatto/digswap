...
import { tradeRequests, tradeReviews } from "@/lib/db/schema/trades";
...
export async function getTrustMetrics(userId: string): Promise<TrustMetrics> {
	...
	const completionRate =
		requestData.total > 0 ? Math.round((requestData.completed / requestData.total) * 100) : 0;
	const responseRate =
		requestData.received > 0 ? Math.round((requestData.responded / requestData.received) * 100) : 0;
