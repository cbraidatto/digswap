...
import { addToCrate, createCrate, getUserCratesAction } from "@/actions/crates";
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent className="w-72 p-4 bg-surface-container border-outline-variant/20">
				<div>
					<p className="font-mono text-xs text-primary tracking-[0.15em] mb-3">[ADD_TO_CRATE]</p>

					{isLoading && (
						<div className="flex items-center gap-2 py-2">
							<span className="w-3 h-3 border border-primary border-t-transparent rounded-full animate-spin" />
							<span className="font-mono text-xs text-on-surface-variant">Loading crates...</span>
						</div>
					)}
					...
					{!isLoading && crates.length > 0 && !showInlineCreate && (
						<div className="space-y-0.5">
							{crates.map((crate) => (
								...
								>
									<span className="font-mono text-xs text-on-surface truncate">{crate.name}</span>
