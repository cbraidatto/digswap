...
export function ImportProgress({ userId, initialJob }: ImportProgressProps) {
	...
	const percent =
		store.totalItems > 0
			? Math.round((store.processedItems / store.totalItems) * 100)
			: 0;
					...
					<Card>
						<CardContent className="p-6 space-y-4">
							...
							<p className="text-sm text-muted-foreground">
								{store.totalItems.toLocaleString()} records
								imported
							</p>
				...
				<div className="mt-[32px] w-full max-w-md mx-auto px-4">
					<Card>
						<CardContent className="p-6 space-y-4">
							...
							<p className="text-sm text-muted-foreground">
								Something went wrong during import. Your
								progress has been saved -- tap retry to continue
								where you left off.
							</p>
							<Button
			...
			<div className="mt-[32px] w-full max-w-md mx-auto px-4">
				<Card>
					<CardContent className="p-6 space-y-4">
						...
						/>
						<p className="text-sm text-muted-foreground">
							{store.processedItems.toLocaleString()} /{" "}
							{store.totalItems.toLocaleString()} records
						</p>
						{store.currentRecord && (
