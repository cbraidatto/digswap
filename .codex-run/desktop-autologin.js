...
(async () => {
  ...
  const runDir = path.join(cwd, '.codex-run');
  ...
  const app = await electron.launch({
    ...
    args: [mainScript],
    cwd: path.join(cwd, 'apps', 'desktop'),
    env,
  });
  ...
  fs.writeFileSync(path.join(runDir, 'desktop-autologin.status.json'), JSON.stringify({ ok: true, email: fixture.requesterEmail, url: page.url(), at: new Date().toISOString() }, null, 2));
  await app.waitForEvent('close');
})().catch((error) => {
  const cwd = process.cwd();
  const runDir = path.join(cwd, '.codex-run');
  fs.writeFileSync(path.join(runDir, 'desktop-autologin.status.json'), JSON.stringify({ ok: false, error: String(error), at: new Date().toISOString() }, null, 2));
  process.exit(1);
});
