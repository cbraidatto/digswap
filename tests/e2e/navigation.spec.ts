...
test.describe("Navigation Shell", () => {
	test.fixme(
		"bottom bar displays 4 tabs with correct labels",
		async ({ page }) => {
	...
	test.fixme(
		"active tab is visually indicated on /feed",
		async ({ page }) => {
			await page.goto("/feed");
			const feedLink = page.locator(
				'nav[aria-label="Main navigation"] a[href="/feed"]',
			);
	...
	test.fixme(
		"deep link to /explorar highlights Explorar tab",
		async ({ page }) => {
			await page.goto("/explorar");
			const explorarLink = page.locator(
				'nav[aria-label="Main navigation"] a[href="/explorar"]',
			);
